...
void _tree_trace( b_node* root, int depth )
{
  ...
  if ( root->left ) sprintf( left_txt, "%5d", root->left->id );
  else              strcpy( left_txt, "<nil>" );

  if ( root->right ) sprintf( right_txt, "%5d", root->right->id );
  else               strcpy( right_txt, "<nil>" );

  if ( root->parent ) sprintf( parent_txt, "%5d", root->parent->id );
  else                strcpy( parent_txt, "<nil>" );

  printf( "lv%3d node:%6d |  l:%s  r:%s  p:%s\n", depth, root->id, left_txt, right_txt, parent_txt );
